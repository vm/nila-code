...
describe('loadRegistry', () => {
  ...
  it('returns null for invalid registry shape', () => {
    const cachePath = join(testDir, 'invalid.json');
    const invalidRegistry = {
      entries: [{ name: 'makepr' }],
      generatedAt: 'not-a-number',
    };

    writeFileSync(cachePath, JSON.stringify(invalidRegistry), 'utf-8');
    const loaded = loadRegistry(cachePath);
...
describe('isRegistryStale', () => {
  ...
  it('returns true when files modified after generatedAt', async () => {
    ...
    const result = isRegistryStale(registry);
